...
<span class="bx_catalog_item_images_img">
	<img src="<?=$img['src']?>" alt="<?=$strTitle?>" class="bx_catalog_item_images_img"></span>
	<?
	if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT'])
	{
	?>
	...
	?>
					...
					<?//при ночной цене скидка не показывается
						   if($arItem['PROPERTIES']['DISCOUNT']['VALUE'] == 1 && !$nightPrice):?>
                          <span><div class="discount-call2"><?php echo '<noindex><p><font size="1"><font color="#B22222">Акция! Позвоните и узнайте скидку у менеджера! Гарантия минимальной цены!</font></font></p></noindex>'?></div></span>
                        <? // значение 2 при ночной цене скидка не показывается и отключает скидку
						elseif($arItem['PROPERTIES']['DISCOUNT']['VALUE'] == 2 && !$nightPrice):?>
                           <span><div class="discount-call2"><?php echo '<noindex><p><font size="1"><font color="#B22222">Акция! Беспатная доставка!</font></font></p></noindex>'?></div></span>
                        <?endif;?>				
						...
						<div class="bx_catalog_item_controls">
							...
							if ($arItem['CAN_BUY'] && !in_array($arItem['DISPLAY_PROPERTIES']['AVAILABILITY']['VALUE_ENUM_ID'], array(4914, 5044, 5649)))
							{
								...
								?>
									...
									if ($arItem['CATALOG_MEASURE_NAME'] == 'кв. м.' && /*!empty($arItem['DISPLAY_PROPERTIES']['SIZE'])*/ !empty($arItem['DISPLAY_PROPERTIES']['SIZE_WIDTH']['VALUE']) && !empty($arItem['DISPLAY_PROPERTIES']['SIZE_LENGTH']['VALUE']) && $arParams['IBLOCK_ID'] == CATALOG_ID) {
