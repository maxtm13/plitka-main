	...
	<? if((($COMMENT['ACTIVATED'] == 0 && $arResult['GROUPS']) || $arResult['ISADMIN']) || $COMMENT['ACTIVATED'] == 1): ?>
		<div class="comment_item <? if($COMMENT['ACTIVATED'] == 0 && ($arResult['GROUPS'] || $arResult['ISADMIN'])): echo "not_approved"; endif; ?>" style="margin-left: <?=$COMMENT['LEFT_MARGIN']?>px;" id="comment_<?=$COMMENT['ID']?>">
			<div <? if($arResult['SHOW_USERPIC'] == "Y"): ?> class="user_icon" <? endif; ?>>
				...
				<? if($COMMENT['USER']['USERLINK'] != ''): ?><p ="<?= $COMMENT['USER']['USERLINK'] ?>"><? endif; ?>
			...
			<? if($COMMENT['USER']['LOGIN']): ?>
				<div class="comment_item_top" <? if($arResult['SHOW_USERPIC'] != "Y"): echo 'style="margin-left:0px;"'; endif; ?>><? if($COMMENT['USER']['USERLINK'] != ''): ?><p ="<?=$COMMENT['USER']['USERLINK'] ?>" alt="<?=$COMMENT['USER']['LOGIN']?>"><?= $COMMENT['USER']['LOGIN']?></p><? else : ?><p><?=$COMMENT['USER']['LOGIN']?></p><? endif; ?><? if($arResult['SHOW_DATE'] == 'Y'): ?>, <?=$COMMENT['DATE_CREATE'] ?><? endif; ?></div>
