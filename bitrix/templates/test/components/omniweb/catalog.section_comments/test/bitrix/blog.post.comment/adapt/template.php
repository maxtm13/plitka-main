...
<div class="blog-comments" id="blg-comment-<?=$arParams["ID"]?>">
	...
	<?php if($arResult["is_ajax_post"] != "Y") {
		...
		<script>
			...
			if(!top.arImagesId)
				top.arImagesId = [];
			<?php foreach($arResult["Images"] as $aImg) { ?>
	...
	if(strlen($arResult["FATAL_MESSAGE"])>0) { ?>
		<div class="blog-errors blog-note-box blog-note-error">
			<div class="blog-error-text"><?=$arResult["FATAL_MESSAGE"]?></div>
		</div>
	<?php } else {
					...
					<div id="form_c_del" style="display:none;">
						<div class="blog-comment-form">
							<form method="POST" name="form_comment" id="form_comment" action="<?=$ajaxPath; ?>">
								...
								<?php if(isset($_REQUEST["IBLOCK_ID"]) || isset($arParams['IBLOCK_ID'])) { ?>
									<input type="hidden" name="IBLOCK_ID" value="<?=(int)((!empty($_REQUEST["IBLOCK_ID"])) ? $_REQUEST["IBLOCK_ID"] : $arParams['IBLOCK_ID']); ?>">
								<?php }
								if(isset($_REQUEST["ELEMENT_ID"]) || isset($arParams['ELEMENT_ID'])) { ?>
									<input type="hidden" name="ELEMENT_ID" value="<?=(int)((!empty($_REQUEST["ELEMENT_ID"])) ? $_REQUEST["ELEMENT_ID"] : $arParams['ELEMENT_ID']); ?>">
								<?php }
								if(isset($_REQUEST["SITE_ID"]) || isset($arParams['SITE_ID'])) { ?>
									<input type="hidden" name="SITE_ID" value="<?=htmlspecialcharsbx((!empty($_REQUEST["SITE_ID"])) ? $_REQUEST["SITE_ID"] : $arParams['SITE_ID']); ?>">
								<?php }
								...
								<div class="blog-comment-fields">
										...
										<div class="blog-comment-field blog-comment-field-user">
											<div class="blog-comment-field blog-comment-field-author">
												<div class="blog-comment-field-text">
													<label for="user_name"><?=GetMessage("B_B_MS_NAME")?></label><span class="blog-required-field">*</span>
												</div>
												<span><input maxlength="255" size="30" tabindex="3" type="text" name="user_name" id="user_name" value="<?=htmlspecialcharsEx($_SESSION["blog_user_name"])?>"></span>
											</div>
											...
											<div class="blog-comment-field blog-comment-field-email">
												...
												<span><input maxlength="255" size="30" tabindex="4" type="text" name="user_email" id="user_email" value="<?=htmlspecialcharsEx($_SESSION["blog_user_email"])?>"></span>
											</div>
									...
									<?php if($arParams["NOT_USE_COMMENT_TITLE"] != "Y") { ?>
										<div class="blog-comment-field blog-comment-field-title">
											<div class="blog-comment-field">
												<div class="blog-comment-field-text">
													<label for="user_name"><?=GetMessage("BPC_SUBJECT")?></label>
												</div>
									...
									if($arResult["COMMENT_PROPERTIES"]["SHOW"] == "Y") { ?>
										...
										foreach($arResult["COMMENT_PROPERTIES"]["DATA"] as $FIELD_NAME => $arPostField) {
											...
											<div id="blog-comment-user-fields-<?=$FIELD_NAME?>">
												<?=($FIELD_NAME=='UF_BLOG_COMMENT_DOC' ? "" : $arPostField["EDIT_FORM_LABEL"].":")?>
			...
			if($arResult["is_ajax_post"] != "Y") {
				if($arResult["CanUserComment"]) {
					...
					<div class="blog-add-comment">
						<a class="bx_medium bx_bt_button" href="javascript:void(0)" onclick="return showComment('0')"><b><?=GetMessage("B_B_MS_ADD_COMMENT")?></b></a><br />
					</div>
					<a name="0"></a>
			...
			if($arResult["is_ajax_post"] != "Y" && $arResult["NEED_NAV"] == "Y") {
				for($i = 1; $i <= $arResult["PAGE_COUNT"]; $i++) {
					...
					$tmp[0] = $arResult["PagesComment"][$i]; ?>
						<div id="blog-comment-page-<?=$i?>"<?if($arResult["PAGE"] != $i) echo "style=\"display:none;\""?>>
...
function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array()) {
	...
	if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview") {
		...
		<a name="<?=$comment["ID"]?>"></a>
		<div class="blog-comment" style="padding-left:<?=$paddingSize?>em;" itemprop="review" itemscope itemtype="http://schema.org/Review">
			<div itemprop="itemReviewed" itemscope itemtype="http://schema.org/Product"><meta itemprop="name" content="<?=$arParams['SECTION_NAME']?>"></div>
			<div id="blg-comment-<?=$comment["ID"]?>">
				<? if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview") {
					...
					<div class="blog-comment-cont<?=$aditStyle?>">
						<div class="blog-comment-cont-white">
							<div class="blog-comment-info">
								...
								<? } else { ?>
									<div class="blog-author" itemprop="author"><?=$comment["AuthorName"]?></div> <!--.blog-author-->
								<? }
								...
								<div class="blog-comment-date">
									<meta itemprop="datePublished" content="<?php echo date('Y-m-d', strtotime($comment["DateFormated"])); ?>">
							...
							<div class="blog-comment-content">
								...
								<div itemprop="reviewBody"><?=$comment["TextFormated"]?></div>
								...
								if($comment["PUBLISH_STATUS"] != BLOG_PUBLISH_STATUS_PUBLISH && $comment["ID"] != "preview") { ?>
									<div class="moderate-info"><?php echo GetMessage('PND_MODERATION'); ?></div>
								<?php } ?>
					...
					</div> <!--.blog-comment-cont<?=$aditStyle?>-->
