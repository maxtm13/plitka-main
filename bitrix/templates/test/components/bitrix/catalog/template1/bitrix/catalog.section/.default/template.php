...
if (!empty($arResult['ITEMS']))
{
	...
	<div class="bx_catalog_list_home col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>" itemscope itemtype="http://schema.org/ItemList">
		...
		foreach ($arResult['ITEMS'] as $key => $arItem)
		{
			...
			$img = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width'=>$w, 'height'=>$h), BX_RESIZE_IMAGE_PROPORTIONAL, true);
			?>
			<div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>" itemscope itemtype="http://schema.org/Product" itemprop="itemListElement">
				...
				<div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
					...
					<a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images" <?/*style="background-image: url(<? echo $img['src']; ?>)"*/?> title="<? echo $strTitle; ?>">
						<?php //if ($img['height'] > $img['width'] && ($img['height'] - $img['width'] > round($img['width'] / 2))) { ?>
							<div class="" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;">
						<?php //} ?>
						<div class="disp-table">
						<div class="disp-table-cell<?php if ($img['height'] > $img['width'] && ($img['height'] - $img['width'] > round($img['width'] / 2))) { echo ' vert-img'; } ?>">
					...
					if ($arItem['SECOND_PICT'])
					{
						...
						$img = CFile::ResizeImageGet($pic, array('width'=>$w, 'height'=>$h), BX_RESIZE_IMAGE_PROPORTIONAL, true);
						?>
						<a id="<? echo $arItemIDs['SECOND_PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="bx_catalog_item_images_double" <?/*style="background-image: url(<? echo $img['src']; ?>)"*/?> title="<? echo $strTitle; ?>">
							<?php //if ($img['height'] > $img['width'] && ($img['height'] - $img['width'] > round($img['width'] / 2))) { ?>
								<div class="" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;">
							<?php //} ?>
							<div class="disp-table">
